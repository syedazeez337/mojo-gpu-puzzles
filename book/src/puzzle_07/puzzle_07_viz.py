...
class Puzzle07Visualization(Scene):
    def construct(self):
        ...
        top_row = VGroup(
            input_matrix[0], Text("...", color=YELLOW), input_matrix[1]
        ).arrange(RIGHT, buff=0.3)
        bottom_row = VGroup(
            input_matrix[2], Text("...", color=YELLOW), input_matrix[3]
        ).arrange(RIGHT, buff=0.3)
        vertical_dots = Text("⋮", color=YELLOW, font_size=24)
        matrix_group = VGroup(top_row, vertical_dots, bottom_row).arrange(
            DOWN, buff=0.3
        )
        matrix_group.move_to(input_bg)
        input_group = VGroup(input_bg, matrix_group)
        input_label = Text("Input Matrix (5×5)", font_size=18).next_to(
        ...
        grid_label = Text(
            "GPU (gridDim.x=2, gridDim.y=2) • (blockDim.x=3, blockDim.y=3)",
            font_size=18,
        ).next_to(grid_bg, UP, buff=0.2)

        # Create 2x2 blocks, each showing 3x3 threads - make blocks smaller
        blocks = VGroup()
        for _block_j in range(2):
            for _block_i in range(2):
                block = VGroup()
                block_bg = Rectangle(
                ...
                for tj, ti in thread_positions:
                    ...
                    thread_text = Text(
                        f"T({tj},{ti})", font_size=10, color=YELLOW
                    )  # Reduced font size
                ...
                top_threads = VGroup(
                    threads[0], Text("...", color=YELLOW), threads[1]
                ).arrange(RIGHT, buff=0.2)
                bottom_threads = VGroup(
                    threads[2], Text("...", color=YELLOW), threads[3]
                ).arrange(RIGHT, buff=0.2)
                thread_dots = Text("⋮", color=YELLOW, font_size=24)
                thread_group = VGroup(
                    top_threads, thread_dots, bottom_threads
        ...
        blocks.arrange_in_grid(
            rows=2, cols=2, buff=0.3
        )  # Reduced buffer from 0.4
        ...
        out_top_row = VGroup(
            output_matrix[0], Text("...", color=YELLOW), output_matrix[1]
        ).arrange(RIGHT, buff=0.3)
        out_bottom_row = VGroup(
            output_matrix[2], Text("...", color=YELLOW), output_matrix[3]
        ).arrange(RIGHT, buff=0.3)
        out_dots = Text("⋮", color=YELLOW, font_size=24)
        output_group = VGroup(out_top_row, out_dots, out_bottom_row).arrange(
            DOWN, buff=0.3
        )
        output_group.move_to(output_bg)
        matrix_output_group = VGroup(output_bg, output_group)
        output_label = Text("Output Matrix (5×5)", font_size=18).next_to(
            matrix_output_group, UP, buff=0.2
        )
            ...
            for block_i in range(2):
                ...
                for tj, ti in thread_positions:
                    ...
                    if global_i >= SIZE or global_j >= SIZE:
                        ...
                        cross = VGroup(
                            Line(
                                start=thread_cell.get_corner(UL)
                                + 0.1 * RIGHT
                                + 0.1 * DOWN,
                                end=thread_cell.get_corner(DR)
                                + (-0.1) * RIGHT
                                + (-0.1) * DOWN,
                                color=RED,
                                stroke_width=2,
                            ...
                            Line(
                                start=thread_cell.get_corner(DL)
                                + 0.1 * RIGHT
                                + (-0.1) * DOWN,
                                end=thread_cell.get_corner(UR)
                                + (-0.1) * RIGHT
                                + 0.1 * DOWN,
                                color=RED,
                                stroke_width=2,
        ...
        block_labels = VGroup(block_idx_label, block_dim_label).arrange(
            RIGHT, buff=0.1
        )
        ...
        self.play(
            Create(grid_bg), Create(blocks), Create(crosses), run_time=1.5
        )
