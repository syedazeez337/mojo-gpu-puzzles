...
class Puzzle08Visualization(Scene):
    def construct(self):
        ...
        for block_idx in range(2):  # 2 blocks
            ...
            shared_label = Text(
                f"Shared Memory (TPB={TPB})", font_size=14, color=WHITE
            )
            ...
            for i in range(TPB):
                cell = Square(
                    side_length=0.7, stroke_width=1, stroke_color=PURPLE_D
                )
            ...
            if block_idx == 0:
                block_label.next_to(block_bg, UP, buff=0.2).shift(
                    LEFT * 0.5
                )  # Subtle left shift
            else:
                block_label.next_to(block_bg, UP, buff=0.2).shift(
