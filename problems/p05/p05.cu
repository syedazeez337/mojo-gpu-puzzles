// CUDA GPU Puzzles - Puzzle 5: Broadcast
//
// Broadcast-add a row vector `a` (shape 1 x SIZE) and a column vector `b`
// (shape SIZE x 1) into a matrix `output` (shape SIZE x SIZE), so that
//     output(row, col) = a(0, col) + b(row, 0)
//
// Broadcasting means a lower-dimensional array is logically stretched to match
// a higher-dimensional one without ever copying data. TensorView makes this
// natural: a (1, N) view reused across rows, an (N, 1) view reused across cols.
//
//   make p05        build & run this file (fails until you fill it in)
//   make test-p05   build & run the reference solution
#include "puzzles.cuh"
...
int main() {
    ...
    // a is a row vector (1 x SIZE), b is a column vector (SIZE x 1).
