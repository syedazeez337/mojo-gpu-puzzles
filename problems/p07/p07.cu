// CUDA GPU Puzzles - Puzzle 7: 2D Blocks
//
// Add 10 to each element of a 5 x 5 matrix using a 2D grid of 2D blocks
// (a 2 x 2 grid of 3 x 3 blocks = 36 threads for 25 elements). Each thread's
// global (row, col) combines its block and in-block coordinates in BOTH
// dimensions:
//     row = blockDim.y * blockIdx.y + threadIdx.y
//     col = blockDim.x * blockIdx.x + threadIdx.x
//
//   make p07        build & run this file (fails until you fill it in)
//   make test-p07   build & run the reference solution
#include "puzzles.cuh"
