// CUDA GPU Puzzles - Puzzle 2: Zip
//
// Add vectors `a` and `b` element-wise into `output`. One thread per position.
// The new idea versus Puzzle 1 is juggling more than one input array in the
// same kernel - each thread reads from the same index in both inputs.
//
//   make p02        build & run this file (fails until you fill it in)
//   make test-p02   build & run the reference solution
#include "puzzles.cuh"
